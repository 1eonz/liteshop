...
### H6. UI 库缺失（已决策：B + ui-kit）

- ~~**A**：按宪法引入（h5 用 antd-mobile 5、admin 用 antd 5）~~
- **B（已选，2026-09-07）**：不引入 antd 系；新建独立 `packages/ui-kit`（Radix headless 底座 + `--ui-*` CSS 变量主题 + react-hook-form 薄层），跨项目可复用
- ~~**C**：暂缓，1b 期再决策~~

> 决策详情见《UI组件库设计方案.md》；连带影响：M7 优先级升为高（ui-kit 前置）、AGENTS.md 技术栈条款需修订（宪法变更，执行前单独确认）。
...
| **4（排期决策）** | ~~H6（UI 库）~~（已定 B，转入 ui-kit 方案 Phase 0-2）、M1/M2（架构收敛，plan-26）、M5/M6（契约变更，需人工 review） | 按 AGENTS.md §8.6 请示 |
