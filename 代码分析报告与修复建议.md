...
本附录以当前工作区为准，覆盖新增的后台运费模板页面、订单地址编辑、`features/*` 领域入口和框架迁移残留，不沿用旧报告的行号结论。

### 11.1 已直接修复

1. **订单地址编辑权限与输入校验**：`AdminService.update_order` 现在只允许 `PENDING_PAYMENT`/`PAID` 修改地址；`OrderManagementUpdate` 校验收货人、手机号和详细地址；新增 10 项回归测试。
2. **运费模板编辑保存失效**：基础模板 PUT 不再携带会被忽略的 `items`；编辑时先更新基础字段，再通过计费项 PUT/POST 保存首个计费项；新建和复制仍通过创建接口提交计费项。
3. **领域入口收敛**：H5 商品查询调用方已改从 `features/catalog/api` 导入，Admin settings/contact 已改从具体领域 service 导入。
4. **迁移残留清理**：确认无内部调用后删除 H5/Admin 根级路由与页面聚合、旧 service/utils 桶、根级防抖转发，以及 Site 旧 `src/page.tsx`。活跃的 `hooks/useDebounceAction.ts`、Admin `utils/format-price.ts` 和兼容出口未误删。

### 11.2 仍然存在但不是本轮擅改项

- 运费模板页面目前只提供首个计费项的编辑，不是完整的多地区计费项增删改；继续扩展应先拆成 model/components/page orchestration，并补服务端事务失败补偿测试。
- `admin-app/src/hooks/useAdminQueries.ts`、`admin-app/src/service/admin.ts`、`h5-app/src/hooks/useProductsQuery.ts` 仍是兼容出口。删除它们会影响潜在包外调用，需先确认兼容窗口和 `no-restricted-imports` 规则。
- `api/admin.py`、`api/orders.py` 和 `page-builder` 仍是较大的历史聚合文件；按域拆分会影响路由标签、契约和导入边界，需单独计划。
- CRM 建议中的权限快照、错误码到缺省页、URL 字典、TTL storage、组件文档、受控/非受控协议已登记为后续候选，不在没有真实调用方时泛化。
- `ui-kit` 只保留并行迁移方案。当前未新增包、未引入重型依赖、未修改 `AGENTS.md` 技术栈；落地前需确认 headless 底座、依赖和 `--ui-*` token 桥接方式。

### 11.3 复核验证

- Backend：`ruff check .`、`mypy app`、`pytest -q`，全量 56 passed；地址编辑回归测试 10 passed。
- H5/Admin/Site：typecheck、lint、Vitest 和各自 build 全部通过。
- E2E：Playwright 5/5 通过。
- Workspace：单独重跑 `pnpm build`、`pnpm lint` 均通过；并行构建曾出现一次 Next.js `.next/server/middleware-manifest.json` 文件竞态，重跑后消失。
