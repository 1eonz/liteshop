...
每个配置包必含 `version` 字段，未来格式升级可兼容：

```markdown
version: 1.0
```

如果 SKILL.md §十 的配置包格式定义升级到 2.0，主 Agent 生成时用新版本号，旧版本号（1.0）的配置包仍可向后兼容读取。
...
**格式说明**：
- `version: 1.0`：配置包版本号，未来格式升级可兼容（见 §4.3.2）
- 5 个必含章节：项目类型识别条件 / 专属风险检测项 / 专属 PRD 关键词 / 专属验收项
