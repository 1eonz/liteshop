...
> 由 project-radar 增量更新：2026-09-10。本文件是主 Agent 的最小上下文入口，子 Agent 若重新启用必须先读取相关章节。
...
- `packages/h5-app/src/components/schema/`：按需加载的首页分类/商品板块与 Schema 属性安全读取工具；`SchemaRenderer` 通过 `React.lazy + Suspense` 加载非首屏数据板块。
...
- `packages/admin-app/src/pages/page-builder/model/page-builder-model.ts`：集中维护搭建器默认 Schema、渠道组件分组、模板、草稿恢复、组件标签和预览工具，页面仅编排交互状态。
- `packages/shared-components/`：提供 `Button`、`EmptyState`、`FeedbackState`、`ErrorState`、`ProductCard` 和唯一的 `useDebounceAction` 实现。
- `packages/ui-kit/`：独立 Phase 1 基础层，提供 Button/Input/EmptyState/Skeleton 与 `pc`/`mobile` 出口；当前仅完成调用方盘点，业务迁移需逐页面进行并保留 shared-components 行为兼容。
...
- ✅ 后端 Service 层直接 ORM 会话调用已清零；订单、售后、退款、RBAC、运费、页面、营销和联系表单的持久化均经由 Repository，`order_workflow` 的 5 个刷新点也已收口。
...
- Admin `useAdminQueries.ts` 与 `service/admin.ts` 仍是兼容出口，真实实现位于各 `features/*/api` 与 `service/admin/<domain>.ts`；后端 `api/admin.py` 与 `api/orders.py` 均已收敛为聚合入口，具体路由分别位于 `api/admin_routes/`、`api/order_routes/`；对应 Service 已按领域拆分。其他 Service 仓储下沉仍待后续，拆分需保持契约快照和路由标签不变。
...
- H5 SchemaRenderer 已将商品/分类数据视图拆为独立 lazy chunk（`components/schema/CategoryGridView.tsx`、`ProductDataView.tsx`），主渲染器首屏不再同步加载数据板块。
