...
**LiteShop 落地方式**：各 service 文件里 URL 字符串目前散落硬编码。建 `packages/shared-types/src/endpoints.ts`：
...
改后端路径只动一处；与 H3（幂等键绑定逻辑动作）改造天然是同一批 PR。
...
**LiteShop 落地方式**（约 20 分钟工作量）：`shared-components` 或各 app utils 下：
...
**LiteShop 落地方式**：引入 antd5（admin）/ antd-mobile（h5）后，`shared-tokens` 成为唯一 token 源，三端派生：

```
tokens.css（源头，修复 M7 后）
  ├─ CSS 变量 --color-*          → Tailwind v4 @theme / 业务样式消费
  ├─ antd5 ConfigProvider theme.token（JS 对象）→ admin 消费
  └─ antd-mobile CSS 变量        → h5 消费
```

这正是 AGENTS.md"禁硬编码颜色"的工程化保障：改 `--color-primary` 一处，三端全生效。

> **2026-09-07 决策更新**：H6 已定为 B（不引入 antd-mobile/antd），上述"三端派生"中 antd 两个分支作废，简化为：`tokens.css → Tailwind v4 / --ui-* 桥接`。通用 UI 层改为独立 ui-kit 包（headless 底座 + CSS 变量主题），详见《UI组件库设计方案.md》第四节——本节的"token 同源"思想不变，载体从 antd 主题变为 `--ui-*` 变量桥接。
...
|---|---|---|---|
| 1 | **useUpdateEffect**（跳过首次渲染的 useEffect，11 行） | useUpdateEffect.js:4-13 | 进 shared-components/hooks；列表筛选"变化才触发请求"标准件 |
...
| H6 UI 库决策后 | 3.1 token 三端派生（CSS 变量 + antd5 token + antd-mobile） | M7 + H6 联动 |
