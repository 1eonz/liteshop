...
| Plan | 范围 | 优先级 | 当前状态 |
|---|---|---|---|
| plan-32 | 售后完整闭环：模型/迁移、7 状态机、H5 申请、Admin 审核、退货入库、幂等与集成测试 | P1 | 已完成基础闭环；关键边界单测 7 项通过，真实 DB 并发仍待补 |
| plan-33 | H5 SchemaRenderer 真实数据消费：首页接 Schema、商品/分类/轮播组件、加载/空/错误态 | P2 | 已完成，首页 Schema、商品/分类数据、状态降级已接入 |
| plan-34 | 官网画布与动画配置：1200px 画布、官网组件面板、动画字段消费、发布预览 E2E | P2 | 已完成主要链路：商城/官网模式、375/1200 画布、SEO/动画字段、reduced-motion |
| plan-35 | Admin UI 缺口：运费模板、评价审核、导航/全局设置、RBAC 管理 | P0/P3 | 核心功能已完成；权限拒绝/编辑冲突/键盘专项 E2E 待补 |
| plan-36 | H5 体验缺口：改地址、支付倒计时、独立搜索、订单步骤条、通知/客服/设置、地址级联 | P0/P3 | 核心功能已完成；i18n、axe-core 键盘/触摸专项测试待补 |
