...
| plan-24 | PostgreSQL/Redis 集成、并发测试与商业验收 | E3.1.2、E12、E15、E16.7 | 本地基础设施、迁移和 9 项真实集成测试（含 5 轮竞态复跑）通过；下架/退款/主题测试与 80% 覆盖率门禁待补 |
...
| plan-26 | 后端分层与关键路径测试收口 | E1.2、E3、E11.5、E15、E16 | 进行中：API/Service/Repository 分层与迁移往返已通过，后端 84 passed、真实集成 9 passed（竞态连续 5 轮）；下架/退款/主题测试和 80% 覆盖率仍待补 |
...
| plan-33 | H5 SchemaRenderer 真实数据消费：首页接 Schema、商品/分类/轮播组件、加载/空/错误态 | P2 | 进行中：刷新/骨架、Schema 商品配置消费、发布响应 E2E、非首屏 lazy/Suspense 已完成；真实数据库发布 E2E 与请求瀑布仍待补 |
| plan-34 | 官网画布与动画配置：1200px 画布、官网组件面板、动画字段消费、发布预览 E2E | P2 | 进行中：主要链路、官网 ISR 鉴权 E2E 和 page-builder 模型/组件/hooks 细拆已完成；真实数据库种子发布/回滚 E2E 待补 |
...
| plan-37 | ui-kit Phase 0/1：独立 token 桥接及最小组件集 | Phase 1 骨架已完成 | 进行中：独立包、LiteShop token 桥接和宿主接入完成；调用方盘点/迁移、弹层 a11y 待补，重依赖与宪法调整需确认 |
...
- 后端：`ruff check`、`ruff format --check`、`python -m mypy .`、`pytest -q` 通过（84 passed；真实 PostgreSQL/Redis 集成 9 passed，覆盖率约 62%，低于 CI 80% 门禁）。
...
- 本地 Docker、PostgreSQL 16、Redis 7 与 Alembic 往返迁移已验证；订单/库存并发、支付竞态、验证码/Token 单次消费、购物车故障、收藏和售后幂等真实集成测试 9 passed，其余下架/退款/主题路径仍待补齐。
...
- 后端 plan-26 已完成全部 Service 直接 ORM 持久化下沉，并完成 Admin 与订单 API/Service 领域拆分；后端 84 passed、覆盖率约 62%；覆盖率 80% 门禁和更深关键路径并发 fixture 仍是上线前工作。
