...
| plan-25 | 前端架构与组件体系收口 | E1.3、E2、E5.4、E15 | 审计完成，待实施；未通过前不得标记商业级验收完成 |
| plan-26 | 后端分层与关键路径测试收口 | E1.2、E3、E11.5、E15、E16 | 审计完成，待实施；与 plan-24 的真实数据库验收配套 |
...
- 前端尚未完成 plan-25：Admin 查询/API 文件职责过重，多个页面仍需拆分，`ProductCard` 与 `formatPrice` 有重复实现，路由未按页面懒加载，i18n 和交互/a11y 测试未落地。
- H5/Admin 尚未安装 AGENTS.md 固定的 Ant Design、Tailwind、lucide、react-hook-form、zod 等依赖；这是既定技术栈与当前实现的偏差，安装和迁移需先取得用户确认。
- 后端尚未完成 plan-26：`api/admin.py`、`services/admin.py`、`api/orders.py` 职责过重，多个 Service 直接操作 ORM；当前 33 条测试总覆盖率 60%，真实数据库关键路径覆盖不足。
