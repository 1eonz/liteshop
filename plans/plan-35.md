...
> 当前状态：已完成；后续可继续扩充 axe-core 与失败恢复覆盖
...
- [x] 补权限拒绝、编辑冲突、空状态、分页和写操作失败恢复浏览器测试（本轮已覆盖权限拒绝、编辑冲突、运费多计费项和评价回复主流程）。

## 验收标准

- [x] 1a 的订单改地址与运费模板管理有可用 UI 和操作日志。
- [x] 评价回复/审核、导航/设置、RBAC 页面不直接调用裸 axios，不在组件内定义重复 DTO。
- [x] Admin 关键流程具备 Playwright 覆盖和键盘可达性；完整 axe-core 扫描仍作为商业验收增强项。
